// Booking seed helpers: provides sample data for demos and customer-specific seeding.
...
export function seedSampleBookings() {
  ...
  try { window.dispatchEvent(new Event('storage')); window.dispatchEvent(new Event('bookingsUpdated')); } catch (err) { console.warn('Could not dispatch events:', err); }
  console.log('Sample bookings seeded:', sampleBookings.length);
  console.log('Total bookings now:', all.length);
  return all;
}
